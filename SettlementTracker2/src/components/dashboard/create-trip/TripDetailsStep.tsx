...
import { TripFormData, DateErrors } from '@/app/(dashboard)/dashboard/create-trip/types/trip';
...
const DateInputs: React.FC<{
  ...
  isEndDateValid: () => boolean;
}> = ({ startDate, endDate, dateErrors, onInputChange, isEndDateValid }) => (
    ...
    <div>
      ...
      <div className="relative mt-1.5">
        ...
        <Input
          ...
          className={cn(
            "pl-9 h-11 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700",
            (dateErrors.endDate || (endDate && !isEndDateValid())) && "border-red-500 focus-visible:ring-red-500"
          )}
        />
      </div>
      {dateErrors.endDate && (
        <p className="text-xs text-red-500 mt-1.5">{dateErrors.endDate}</p>
      )}
      {!dateErrors.endDate && endDate && !isEndDateValid() && (
