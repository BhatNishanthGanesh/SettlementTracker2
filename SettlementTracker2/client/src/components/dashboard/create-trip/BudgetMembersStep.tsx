...
import { UserPlus, User, X, Sparkles } from 'lucide-react';
import { Member } from '@/app/(dashboard)/dashboard/create-trip/types/trip';
...
export const BudgetMembersStep: React.FC<BudgetMembersStepProps> = ({
  budget,
  members,
  sessionName,
  sessionEmail,
...
const BudgetInput: React.FC<{
  ...
  <div>
    ...
    <p className="text-xs text-muted-foreground mt-1.5 flex items-center gap-1">
      ...
      AI will track spending and alert you if you go over budget
    </p>
  </div>
);
...
const MembersSection: React.FC<{
  ...
  onRemoveMember: (index: number) => void;
  getTotalMembers: () => number;
}> = ({ members, sessionName, sessionEmail, onMemberChange, onAddMember, onRemoveMember, getTotalMembers }) => (
  <div>
    ...
    <p className="text-xs text-muted-foreground mt-2">
      <span className="text-emerald-500">✉️</span> Members with email will receive an invitation
    </p>
  </div>
);
