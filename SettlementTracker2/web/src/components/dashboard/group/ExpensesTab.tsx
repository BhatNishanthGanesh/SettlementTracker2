...
'use client';
import { Plus, Wallet, Receipt, User, Calendar, Tag, Trash2 } from 'lucide-react';
  ...
  const handleAddExpense = async (formData: ExpenseFormData) => {
    ...
    try {
      ...
      await expenseService.createExpenseMessage(trip.id, {
        ...
        splitBetween: formData.splitBetween,
        splitBetweenNames: splitMembers.map(
          m => m?.name || "Unknown"
        ),
        splits: formData.splits,
        category: formData.category,
...
function ExpenseItem({
  ...
  const getSplits = (): Record<string, number> => {
...
const perPersonAmount =
  currentUserId && splits[currentUserId] !== undefined
