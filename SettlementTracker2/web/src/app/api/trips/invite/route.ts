...
export async function POST(request: NextRequest) {
  try {
    ...
    const { tripId, tripName, members } = body;
    ...
    const joinLink = `${process.env.NEXT_PUBLIC_APP_URL}/join/${trip.inviteCode}`;

    const emailPromises = members
      .filter((member: any) => member.email && member.email !== user.email)
              ...
              <body>
                <div class="container">
                  ...
                  <div class="content">
                    ...
                    <p><strong>${user.name}</strong> has invited you to join the trip <strong>"${tripName}"</strong>.</p>
                    
                    <div class="trip-details">
                      <p style="margin: 5px 0;"><strong>Trip:</strong> ${tripName}</p>
                      <p style="margin: 5px 0;"><strong>Organized by:</strong> ${user.name}</p>
    ...
    return NextResponse.json({
      ...
      sentCount,
      total: members.filter((m: any) => m.email && m.email !== user.email).length,
